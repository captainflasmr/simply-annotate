...
- *Fringe bracket style* -- New =fringe-bracket= display draws a vertical bracket in the fringe spanning the annotated region.
- *Tint style* -- New =tint= display applies a subtle background tint derived from the current theme.
- *Reply indicator* -- Replies show =↳= before the author line in both inline and buffer views.
- *Consistent formatting* -- Inline and =*Annotation*= buffer use the same compact comment layout.
- *Overlay detection fix* -- =simply-annotate-smart-action= works from any line within an annotated region.
