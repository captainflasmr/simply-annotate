...
- *Nested replies* -- =M-s r= now prompts which comment to reply to, enabling true threaded conversations with arbitrary nesting depth.
...
- =simply-annotate-mode-map= -- The minor mode keymap, active when the mode is on.  All bindings use =M-s= prefixed keys (e.g. =M-s j=, =M-s r=) to avoid conflicts.
- =simply-annotate-command-map= -- A standalone prefix keymap with short single-key bindings (=j=, =r=, =n=, etc.) designed to be bound to a prefix key.

*** Default (M-s prefix)

With the default setup, all commands are available via =M-s= when the mode is active:
...
*** Custom prefix key (recommended for short keys)

Bind =simply-annotate-command-map= to any prefix for short-key access (e.g., =C-c a j=, =C-c a n=):
...
Cycle individual styles with =M-s '= or customize =simply-annotate-display-style=.
