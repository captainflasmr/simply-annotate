...
- *Annotation levels* -- Three levels: =file=, =defun=, =line=.  Cycle with =M-s [=.
- *Level-scoped interaction* -- Navigation, header counts, and editing are filtered to the active level.  All visual markers remain visible across levels.
- *Internal cleanup* -- Private functions use double-dash naming, buffer-local state fixes, improved error handling.
