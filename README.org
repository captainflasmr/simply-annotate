...
- *Org-mode listing* -- =M-s l= now uses org-mode with full folding.  Annotations, regions, and comments each have their own collapsible headings.
- *Global overview* -- =simply-annotate-show-all= shows all annotations across all files in a single org buffer with file-level folding.
- *Quick file jump* -- =simply-annotate-jump-to-file= provides completing-read navigation for fast file switching.
- *Listing navigation* -- =n= / =p= step between annotations and jump to source.  =RET= to jump, =q= to quit.
- *Live refresh* -- Listing buffer updates automatically when annotations are modified.
- *Full region capture* -- Listing shows the complete annotated region, not just the first line.
