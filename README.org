...
- *Dired integration* -- =simply-annotate-dired-mode= shows fringe indicators next to annotated files in dired.
- *All-levels display* -- New =all= pseudo-level shows every annotation.  Now the default on startup.
- *Inline word-wrap* -- Inline comments word-wrap to =simply-annotate-inline-fill-column= (default 72).
- *Compact rendering* -- No emoji icons, embedded position in header (=DEFUN: 3/5=), silent startup.
- *C-g to dismiss* -- =C-g= in annotation buffer cancels/hides like =C-c C-k=.
