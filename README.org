...
- *Inline display* -- =M-s /= toggles box-drawn annotation blocks in the buffer.  Position with =simply-annotate-inline-position= (='after= or ='above=).
- *Subtle display style* -- New overline/underline style that brackets annotated regions unobtrusively.
- *Level-filtered display* -- Display styles now show only annotations at the active level.
- *Redesigned listing* -- =M-s l= groups annotations by level with counts and auto-syncs level on jump.
- *Key binding changes* -- =M-s '= for display style, =M-s [=/=]= for level cycling, =M-s /= for inline toggle.
