...
r (reply), M-s l (list), etc. All keybindings in this manual assume the
M-s prefix.
...
- =bar= :: Draws a solid heavy vertical bar (=┃ =) in the left margin of every annotated line.

- =bracket= :: Outlines annotated regions with heavy box-drawing corners (=┏ =, =┃ =, =┗ =) for multi-line annotations, or a single =┠ = indicator for single-line annotations. This is now the default display style.
...
Emacs built-in =project.el= (available since Emacs 28.1).
